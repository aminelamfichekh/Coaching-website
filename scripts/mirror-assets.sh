#!/usr/bin/env bash
# Mirrors assets discovered in /tmp/asset-urls.txt into docs/assets/.
...
while IFS= read -r url; do
  ...
  case "$rel" in
    wp-content/themes/grit/*)
      ...
      src="grit/grit/$sub"
      dst="docs/assets/theme/$sub"
      if [ -f "$src" ]; then
        mkdir -p "$(dirname "$dst")"
        cp "$src" "$dst"
        echo "theme-local: $sub" >> "$LOCAL_LOG"
      else
        echo "$url|docs/assets/theme/$sub" >> "$REMOTE_QUEUE"
    ...
    wp-content/plugins/booked/*)
      ...
      dst="docs/assets/plugins/booked/$sub"
      if [ -f "$src" ]; then
        mkdir -p "$(dirname "$dst")"
        cp "$src" "$dst"
        echo "booked-local: $sub" >> "$LOCAL_LOG"
      else
        echo "$url|docs/assets/plugins/booked/$sub" >> "$REMOTE_QUEUE"
    ...
    wp-content/plugins/revslider/*)
      ...
      dst="docs/assets/plugins/revslider/$sub"
      if [ -f "$src" ]; then
        mkdir -p "$(dirname "$dst")"
        cp "$src" "$dst"
        echo "revslider-local: $sub" >> "$LOCAL_LOG"
      else
        echo "$url|docs/assets/plugins/revslider/$sub" >> "$REMOTE_QUEUE"
    ...
    wp-content/plugins/*)
      ...
      echo "$url|docs/assets/plugins/$sub" >> "$REMOTE_QUEUE"
      ;;
    wp-content/uploads/*)
      sub="${rel#wp-content/uploads/}"
      echo "$url|docs/assets/images/uploads/$sub" >> "$REMOTE_QUEUE"
      ;;
    wp-includes/*)
      sub="${rel#wp-includes/}"
      echo "$url|docs/assets/vendor/wp-includes/$sub" >> "$REMOTE_QUEUE"
      ;;
    *)
