...
if ( ! function_exists( 'grit_storage_get' ) ) {
	/**
	 ...
	 */
	function grit_storage_get( $var_name, $default = '' ) {
		global $GRIT_STORAGE;
		return isset( $GRIT_STORAGE[ $var_name ] ) ? $GRIT_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'grit_storage_set' ) ) {
	/**
	 ...
	 */
	function grit_storage_set( $var_name, $value ) {
		global $GRIT_STORAGE;
		$GRIT_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'grit_storage_empty' ) ) {
	/**
	 ...
	 */
	function grit_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $GRIT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $GRIT_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $GRIT_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'grit_storage_isset' ) ) {
	/**
	 ...
	 */
	function grit_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $GRIT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $GRIT_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $GRIT_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'grit_storage_unset' ) ) {
	/**
	 ...
	 */
	function grit_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $GRIT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $GRIT_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $GRIT_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'grit_storage_inc' ) ) {
	/**
	 ...
	 */
	function grit_storage_inc( $var_name, $value = 1 ) {
		global $GRIT_STORAGE;
		if ( empty( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = 0;
		}
		$GRIT_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'grit_storage_concat' ) ) {
	/**
	 ...
	 */
	function grit_storage_concat( $var_name, $value ) {
		global $GRIT_STORAGE;
		if ( empty( $GRIT_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'grit_storage_get_array' ) ) {
	/**
	 ...
	 */
	function grit_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $GRIT_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $GRIT_STORAGE[ $var_name ][ $key ] ) ? $GRIT_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'grit_storage_set_array' ) ) {
	/**
	 ...
	 */
	function grit_storage_set_array( $var_name, $key, $value ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$GRIT_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'grit_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function grit_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $GRIT_STORAGE[ $var_name ][ $key ] ) ) {
			$GRIT_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$GRIT_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$GRIT_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'grit_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function grit_storage_merge_array( $var_name, $key, $value ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$GRIT_STORAGE[ $var_name ] = array_merge( $GRIT_STORAGE[ $var_name ], $value );
		} else {
			$GRIT_STORAGE[ $var_name ][ $key ] = array_merge( $GRIT_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'grit_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function grit_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			grit_array_insert_after( $GRIT_STORAGE[ $var_name ], $after, $key );
		} else {
			grit_array_insert_after( $GRIT_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'grit_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function grit_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			grit_array_insert_before( $GRIT_STORAGE[ $var_name ], $before, $key );
		} else {
			grit_array_insert_before( $GRIT_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'grit_storage_push_array' ) ) {
	/**
	 ...
	 */
	function grit_storage_push_array( $var_name, $key, $value ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $GRIT_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $GRIT_STORAGE[ $var_name ][ $key ] ) ) {
				$GRIT_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $GRIT_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'grit_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function grit_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $GRIT_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $GRIT_STORAGE[ $var_name ] ) && is_array( $GRIT_STORAGE[ $var_name ] ) && count( $GRIT_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $GRIT_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $GRIT_STORAGE[ $var_name ][ $key ] ) && is_array( $GRIT_STORAGE[ $var_name ][ $key ] ) && count( $GRIT_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $GRIT_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'grit_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function grit_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( empty( $GRIT_STORAGE[ $var_name ][ $key ] ) ) {
			$GRIT_STORAGE[ $var_name ][ $key ] = 0;
		}
		$GRIT_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'grit_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function grit_storage_concat_array( $var_name, $key, $value ) {
		global $GRIT_STORAGE;
		if ( ! isset( $GRIT_STORAGE[ $var_name ] ) ) {
			$GRIT_STORAGE[ $var_name ] = array();
		}
		if ( empty( $GRIT_STORAGE[ $var_name ][ $key ] ) ) {
			$GRIT_STORAGE[ $var_name ][ $key ] = '';
		}
		$GRIT_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'grit_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function grit_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $GRIT_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $GRIT_STORAGE[ $var_name ] ) ? $GRIT_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $GRIT_STORAGE[ $var_name ] ) ? $GRIT_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'grit_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function grit_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $GRIT_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $GRIT_STORAGE[ $var_name ]->$prop ) ? $GRIT_STORAGE[ $var_name ]->$prop : $default;
	}
}
