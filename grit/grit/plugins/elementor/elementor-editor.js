...
jQuery( document ).ready(
	function() {
		...
		grit_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="grit_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.grit_post_editor' );
						if ( link.length > 0 ) {
							grit_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.grit_post_editor', function(e) {
						grit_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('grit_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function grit_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
