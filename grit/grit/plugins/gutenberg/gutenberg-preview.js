...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			grit_create_observer( 'grit-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				grit_gutenberg_editor_init();
			} );
		} else {
		 	grit_gutenberg_editor_init();
		}
	...
	function grit_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'grit_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'grit_inited' ) ) {
	...
	// Init on page load
	function grit_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = grit_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + GRIT_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + GRIT_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in GRIT_STORAGE['override_classes'] ) {
				$obj.addClass( GRIT_STORAGE['override_classes'][i].replace( '%s', GRIT_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( grit_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && GRIT_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'grit_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			grit_remove_observer( 'grit-check-editor-styles-wrapper' );
			grit_create_observer( 'grit-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'grit_inited' );
	}
} );
