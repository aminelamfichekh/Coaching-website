<div class="front_page_section front_page_section_features<?php
	$grit_scheme = grit_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $grit_scheme ) && ! grit_is_inherit( $grit_scheme ) ) {
		echo ' scheme_' . esc_attr( $grit_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( grit_get_theme_option( 'front_page_features_paddings' ) );
	if ( grit_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$grit_css      = '';
		$grit_bg_image = grit_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$grit_anchor_icon = grit_get_theme_option( 'front_page_features_anchor_icon' );
	$grit_anchor_text = grit_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( grit_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$grit_css      = '';
			$grit_bg_mask  = grit_get_theme_option( 'front_page_features_bg_mask' );
			$grit_bg_color_type = grit_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $grit_bg_color_type ) {
				$grit_bg_color = grit_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$grit_caption = grit_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $grit_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $grit_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $grit_caption, 'grit_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$grit_description = grit_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $grit_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $grit_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $grit_description ), 'grit_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! grit_exists_trx_addons() ) {
						...
						grit_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
