<div class="front_page_section front_page_section_team<?php
	$grit_scheme = grit_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $grit_scheme ) && ! grit_is_inherit( $grit_scheme ) ) {
		echo ' scheme_' . esc_attr( $grit_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( grit_get_theme_option( 'front_page_team_paddings' ) );
	if ( grit_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$grit_css      = '';
		$grit_bg_image = grit_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$grit_anchor_icon = grit_get_theme_option( 'front_page_team_anchor_icon' );
	$grit_anchor_text = grit_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( grit_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$grit_css      = '';
			$grit_bg_mask  = grit_get_theme_option( 'front_page_team_bg_mask' );
			$grit_bg_color_type = grit_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $grit_bg_color_type ) {
				$grit_bg_color = grit_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$grit_caption = grit_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $grit_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $grit_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $grit_caption, 'grit_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$grit_description = grit_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $grit_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $grit_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $grit_description ), 'grit_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! grit_exists_trx_addons() ) {
						...
						grit_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
