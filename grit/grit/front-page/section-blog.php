<div class="front_page_section front_page_section_blog<?php
	$grit_scheme = grit_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $grit_scheme ) && ! grit_is_inherit( $grit_scheme ) ) {
		echo ' scheme_' . esc_attr( $grit_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( grit_get_theme_option( 'front_page_blog_paddings' ) );
	if ( grit_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$grit_css      = '';
		$grit_bg_image = grit_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$grit_anchor_icon = grit_get_theme_option( 'front_page_blog_anchor_icon' );
	$grit_anchor_text = grit_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( grit_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$grit_css      = '';
			$grit_bg_mask  = grit_get_theme_option( 'front_page_blog_bg_mask' );
			$grit_bg_color_type = grit_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $grit_bg_color_type ) {
				$grit_bg_color = grit_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$grit_caption = grit_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $grit_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $grit_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $grit_caption, 'grit_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$grit_description = grit_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $grit_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $grit_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $grit_description ), 'grit_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! grit_exists_trx_addons() ) {
						...
						grit_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
