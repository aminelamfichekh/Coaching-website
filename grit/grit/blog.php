...
if ( function_exists( 'grit_elementor_is_preview' ) && grit_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			grit_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}
	...
	grit_new_main_query(
		array(
			'post_type'      => grit_get_theme_option( 'post_type' ),
			'category'       => grit_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'grit_filter_get_template_part', grit_blog_archive_get_template() ) );
}
