...
// Additional info
if ( grit_is_off( grit_get_theme_option( 'header_mobile_hide_info' ) ) ) {
	$grit_info = grit_get_theme_option( 'header_mobile_additional_info' );
	if ( '' != $grit_info ) {
	...
	<div class="content_wrap">
		<div class="columns_wrap columns_fluid">
			<div class="sc_layouts_column sc_layouts_column_align_left sc_layouts_column_icons_position_left sc_layouts_column_fluid column-1_3">
				<?php
				// Logo
				if ( grit_is_off( grit_get_theme_option( 'header_mobile_hide_logo' ) ) ) {
					?>
					<div class="sc_layouts_item">
						<?php
						set_query_var( 'grit_logo_args', array( 'type' => 'mobile_header' ) );
				...
				if ( grit_exists_trx_addons() ) {
					...
					if ( grit_is_off( grit_get_theme_option( 'header_mobile_hide_login' ) ) ) {
						ob_start();
						do_action(
							'grit_action_login', array(
								'text_login'  => false,
								'text_logout' => false,
							)
						);
						$grit_action_output = ob_get_contents();
						ob_end_clean();
						if ( ! empty( $grit_action_output ) ) {
							?>
								<div class="sc_layouts_item sc_layouts_menu sc_layouts_menu_default">
										<?php
										grit_show_layout( $grit_action_output );
										?>
								</div>
										<?php
						}
					...
					// Display cart button
					if ( grit_is_off( grit_get_theme_option( 'header_mobile_hide_cart' ) ) ) {
						ob_start();
						do_action( 'grit_action_cart' );
						$grit_action_output = ob_get_contents();
						ob_end_clean();
						if ( ! empty( $grit_action_output ) ) {
							?>
					...
					if ( grit_is_off( grit_get_theme_option( 'header_mobile_hide_search' ) ) ) {
						ob_start();
						do_action(
							'grit_action_search',
							array(
